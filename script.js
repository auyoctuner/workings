...
function renderJobs(jobs) {
    ...
    if (!jobs || jobs.length === 0) {
        jobsList.innerHTML = `<tr><td colspan="8" class="p-4 text-center text-gray-500">ไม่มีรายการ</td></tr>`;
        updateCheckAllState();
        return;
    }
    let currentDay = null;
    jobs.forEach(job => {
        ...
        if (dateStr !== currentDay && currentDay !== null) {
            jobsList.insertAdjacentHTML('beforeend', `<tr><td colspan="8"><hr class="my-4 border-t-2 border-blue-400"></td></tr>`);
        }
        ...
        // คำนวณกำไร
        let profitHTML = '';
        if (typeof job.revenue === 'number' && typeof job.cost === 'number') {
            const profit = job.revenue - job.cost;
            profitHTML = `<span class="${profit >= 0 ? 'text-blue-600' : 'text-red-600'}">${profit.toLocaleString()}</span>`;
        } else if (job.revenue === 'claim' || job.revenue === 'refund' || (typeof job.revenue === 'number' && job.revenue < 0)) {
            profitHTML = `<span class="text-gray-400">-</span>`;
        } else {
            profitHTML = `<span class="text-yellow-500 font-medium">รอดำเนินการ</span>`;
        }
        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 text-sm text-center">${profitHTML}</td>
